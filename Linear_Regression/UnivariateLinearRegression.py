...
class UnivariateLinearRegression:
    def __init__(self, data, labels, alpha, num_iterations):
        """
        1.对数据进行预处理操作
        2.先得到所有的特征个数
        3.初始化参数矩阵
    ...
    def normalize(self, data):
        ...
        ini_data = [i[0] for i in data]
        temp = (data - np.mean(ini_data)) / np.std(ini_data)
        data_process.append(list(temp))
    ...
    def cost_function(self):
        ...
        损失计算方法
        """
        delta = UnivariateLinearRegression.hypothesis(
    ...
    def predict(self, data, labels, theta):
        ...
        predictions = UnivariateLinearRegression.hypothesis(
            data_process, theta)
    ...
    def gradient_step(self):
        ...
        """
        prediction = UnivariateLinearRegression.hypothesis(
